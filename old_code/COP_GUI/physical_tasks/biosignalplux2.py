from physical_tasks import plux
        ...
        if PluxLoggingFlag:
            if plux_file is not None:
                ...
                if 'data2' not in globals():
                    ...
                    # data2.append(data)
                # plux_file.write("{0:f},{1:f},{2:f},{3:f},{4:f}\n".format(time.time(), data[0], data[1], data[2], data[3]))
                plux_file.write("{0:s},{1:f},{2:f},{3:f},{4:f},{5:f},{6:f}\n".format(timestr, data[0], data[1], data[2], data[3], data[4], data[5]))
            else:
...
def PluxDataFile(path):
    global plux_file

    try:
        current_time = datetime.datetime.now().strftime('%Y-%m-%d__%H.%M.%S.%f')
        _filename = "%s/Plux-%s.csv" % (path, current_time)
        if plux_file is not None:
            plux_file.close()
            plux_file = None

        plux_file = open(_filename, "a")
...
def PluxConnect():
    ...
    try:
        dev = MyDevice("00:07:80:46:E5:C0")  # MAC address of device
        ...
        red = 2
        ired = 2
        dev.setParameter(9, 3, chr(red) + chr(ired))
        ...
        src_spo2 = plux.Source()
        src_spo2.port = 9
        src_spo2.chMask = 3

        dev.start(1000, (src_a, src_b, src_c, src_d, src_spo2))


        # dev.start(1000, 0x10F, 16)  # 1000 Hz, ports 1-2, 16 bits
...
#
# 2/13/21 Had to update print statements to python3 standards -JL
